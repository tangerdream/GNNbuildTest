...
class Analyzer:
    ...
    def __init__(self,
        ...
        self.tb_writer        = SummaryWriter(log_dir=self.constants.tensorboard_dir,
                                              flush_secs=10)
    ...
    def evaluate_model(self, likelihood_per_action : torch.Tensor) -> None:
        ...
        avg_gen_likelihood = (
            torch.sum(likelihood_per_action) / self.constants.n_samples
        )
        ...
        util.write_validation_scores(
            output_dir=self.constants.job_dir,
            epoch_key=epoch_key,
            model_scores=model_scores,
            append=bool(epoch_key != f"{epoch_label} {self.constants.sample_every}")
        )
    ...
    def evaluate_generated_graphs(self, generated_graphs : list,
        ...
        if generation_batch_idx == 0:
            output = self.constants.job_dir  # shorthand
    ...
    def get_molecular_properties(self, molecules : list, epoch_key : str,
        ...
        def _get_n_nodes_distribution(molecular_graphs : list) -> \
            ...
            n_nodes_histogram = torch.zeros(self.constants.max_n_nodes + 1,
                                            device=self.constants.device)
        ...
        def _get_node_feature_distribution(molecular_graphs : list) -> \
            ...
            if isinstance(molecular_graphs[0].node_features, torch.Tensor):
                nodes_hist = torch.zeros(self.constants.n_node_features,
                                         device=self.constants.device)
            else:
                nodes_hist = np.zeros(self.constants.n_node_features)
            ...
            idc = util.get_feature_vector_indices(self.constants)  # **note: "idc" == "indices"
            ...
            if not self.constants.use_explicit_H and not self.constants.ignore_H:
                ...
                numh_histogram      = [0] * self.constants.n_imp_H
            if self.constants.use_chirality:
                ...
                chirality_histogram = [0] * self.constants.n_chirality
        ...
        def _get_edge_feature_distribution(molecular_graphs : list) -> \
            ...
            edge_feature_hist = torch.zeros(self.constants.n_edge_features,
                                            device=self.constants.device)

            for molecular_graph in molecular_graphs:
                edges = molecular_graph.edge_features
                for edge in range(self.constants.n_edge_features):
    ...
    def combine_ts_properties(self, prev_properties : dict,
        ...
        ----
            ...
            weight_next (int)      : Weight given to `next_properties`, equal to
                                     ...
                                     it (the weight is assumed to be `self.constants.batch_size`
                                     for `prev_properties`).
        ...
        weight_prev = self.constants.batch_size
    ...
    def get_validation_likelihood(self, dataset : str) -> \
        ...
        n_samples    = min(100000, self.constants.n_samples)  # n graphs to evaluate
        likelihoods  = torch.zeros(n_samples * (self.constants.max_n_nodes+5),
                                   device=self.constants.device)
        n_structures = torch.zeros(1, device=self.constants.device)

        # `batch` contains self.constants.n_samples subgraphs during validation
        for idx, batch in enumerate(dataloader):
            ...
            if idx * self.constants.batch_size > n_samples:
                break
            ...
            start_idx  = idx * self.constants.batch_size
            end_idx    = idx * self.constants.batch_size + len(likelihood)
    ...
    def plot_molecular_properties(self, properties : dict,
        ...
        for epoch_key in keys_to_plot:
            ...
            ax_at.plot(range(1, len(norm_atom_type_hist) + 1), norm_atom_type_hist,
                       color=c, label=epoch_key, linestyle=ls, marker=m)
            xlabel_values = ", ".join(map(str, self.constants.atom_types))
            ax_at.set(xlabel=f"Atom type ({xlabel_values})")

            # plot formal charge histogram
            ax_fc.plot(range(self.constants.formal_charge[0], self.constants.formal_charge[-1] + 1),
                       norm_formal_charge_hist,
                       color=c, label=epoch_key, linestyle=ls, marker=m)
            xlabel_values = ", ".join(map(str, self.constants.formal_charge))
            ax_fc.set(xlabel=f"Formal charge ({xlabel_values})")

            # plot num H histogram
            ax_nh.plot(self.constants.imp_H, norm_numh_hist,
                       color=c, label=epoch_key, linestyle=ls, marker=m)
            xlabel_values = ", ".join(map(str, self.constants.imp_H))
            ...
            ax_bt.plot(range(0, len(norm_edge_feature_hist)),
                       ...
                       color=c, label=epoch_key, linestyle=ls, marker=m)
            xlabel_values = ", ".join(map(str, self.constants.int_to_bondtype))
            ...
            ax_ct.plot(range(1, len(norm_chirality_hist) + 1), norm_chirality_hist,
                       color=c, label=epoch_key, linestyle=ls, marker=m)
            xlabel_values = ", ".join(map(str, self.constants.chirality))
    ...
    def save_metrics(self, step, score, append=True) -> None:
        ...
        """
        if not append:
            with open(self.constants.job_dir + "fine-tuning.log", "w") as output_file:
                output_file.write("Step, Score\n")
                output_file.write(f"Step {step}, {score:.8f}\n")
        else:
            with open(self.constants.job_dir + "fine-tuning.log", "a") as output_file:
                output_file.write(f"Step {step}, {score:.8f}\n")
