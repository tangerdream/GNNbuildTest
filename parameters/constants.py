...
def load_params(input_csv_path : str) -> dict:
    ...
    """
    params_to_override = {}
    with open(input_csv_path, "r",newline='\n') as csv_file:
...
def override_params(all_params : dict) -> dict:#覆写默认参数
    """
...
def collect_global_constants(parameters : dict, job_dir : str) -> namedtuple:
    ...
    n_edge_features = len(bondtype_to_int)#边的特征数，只论类型数量

    # define node feature constants
    n_atom_types, n_formal_charge, n_imp_H, n_chirality = get_feature_dimensions(parameters)

    n_node_features = n_atom_types + n_formal_charge + n_imp_H + n_chirality#结点数
    ...
    if constants_dict["job_type"] != "preprocess":
        ...
        # load preprocessing parameters for comparison (if they exist already)#参数校验
